    ...
    >
      <Star
        className={`h-4 w-4 transition-colors ${
          saved ? 'fill-yellow-400 text-yellow-400' : 'text-slate-400 hover:text-slate-200'
        }`}
      />
    </button>
  );
        ...
        {!loading && weather && condition && (
          <>
            <div className="mb-4 flex items-start gap-3">
              <span className="text-5xl leading-none" role="img" aria-label={condition.label}>
                {condition.emoji}
              </span>
              <div className="flex-1">
                ...
                <div className="mt-1 text-xs text-slate-400">{condition.label}</div>
              </div>
